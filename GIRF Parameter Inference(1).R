...
u = function(m,sss,j) {
  inter1 = 1-sss/SSS
  Int1 = function(z,k) dnbinom(z,inter1/sigma2,1/(sigma2*v.ir*X[m,sss,j,3+(k-1)*5])) *  dmeas(y[(m+1),(k+1)],rho,phi,z)
  ...
  inter2 = 2-sss/SSS
  Int2 = function(z,k) dnbinom(z,inter2/sigma2,1/(sigma2*v.ir*X[m,sss,j,3+(k-1)*5])) *  dmeas(y[(m+2),(k+1)],rho,phi,z) 
...
for (mm in 1:MM) {
  ...
  for(m in 1:M){
    if(m==1){
      ...
      div = sapply(1:J,function(j) prod(sapply(1:3, function(k) dmeas(y[(m),(k+1)],rho,phi,X[m-1,SSS+1,j,4+(k-1)*5]))))
    }
    for (sss in 1:SSS) {
      #Propsed particles
      for (j in 1:J) {
