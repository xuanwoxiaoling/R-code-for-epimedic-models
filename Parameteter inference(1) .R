    ...
    for (l in 1:3) {
      if(l==k){
        ...
        ss[l] = travel_prob * vkl[l]/tot_travel *
          ((X[mm,m,j,3+(l-1)*5]/as.numeric(N[round(year[t]-start.year+0.5),(l+1)]))^alpha - (X[mm,m,j,3+(k-1)*5]/as.numeric(N[round(year[t]-start.year+0.5),(k+1)]))^alpha)
      }
    }
...
theta.10 = array(0,dim = c((MM+1),(M+1),J))
theta.11 = array(0,dim = c((MM+1),(M+1),J))
theta.12 = array(0,dim = c((MM+1),(M+1),J))

#theta.1[1,1,] = runif(J,0,1)
#theta.2[1,1,] = runif(J,0,1)
#theta.3[1,1,] = runif(J,0,1)
#theta.4[1,1,] = runif(J,0,1)
#theta.5[1,1,] = runif(J,0,1)
#theta.6[1,1,] = runif(J,0,1)
#theta.7[1,1,] = runif(J,0,1)
#theta.8[1,1,] = runif(J,0,1)
#theta.9[1,1,] = runif(J,0,1)
theta.10[1,1,] = rnorm(J,-3,1)
theta.11[1,1,] = sapply(1:J,function(j) min(rnorm(1,-8,1),-theta.10[1,1,j]))
theta.12[1,1,] = sapply(1:J,function(j) min(rnorm(1,-8,1),
...
for (mm in 15:MM) {
  s = sapply(1:J,function(j) logit(theta.10[mm,1,j]))
  e = sapply(1:J,function(j) logit(theta.11[mm,1,j]))
  i = sapply(1:J,function(j) logit(theta.12[mm,1,j]))
  ...
  for(m in 1:M){ #when estimate s.e.i only use the data from initial couples of weeks we choose firt year 
      ...
      for (j in 1:J) {
        ...
        if(m==1){
          # initial X
          for (k in 1:3) {
            X[mm,1,j,1+(k-1)*5] = round(s[j]*N[round(year[T]-start.year+0.5),(k+1)]) #S
            X[mm,1,j,2+(k-1)*5] = round(e[j]*N[round(year[T]-start.year+0.5),(k+1)]) #E
            X[mm,1,j,3+(k-1)*5] = round(i[j]*N[round(year[T]-start.year+0.5),(k+1)]) #I
            X[mm,1,j,4+(k-1)*5] = 0   #delta N.ir(weekly)
            X[mm,1,j,5+(k-1)*5] = N[round(year[T]-start.year+0.5),(k+1)] #P 
          }
        }

        #perturbation density for parameters is Norm with sd based on the geometric cooling scheme
        theta.10[mm,m+1,] = sapply(1:J,function(j) theta.10[mm,m+1,j] = theta.10[mm,m,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm > 50,0.0008,0.003))))
        theta.11[mm,m+1,] = sapply(1:J,function(j) theta.11[mm,m+1,j] = min(theta.11[mm,m,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm>50,0.0005,0.002))),-theta.10[mm,m+1,j]))
        theta.12[mm,m+1,] = sapply(1:J,function(j) theta.12[mm,m+1,j] = min(theta.12[mm,m,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm>50,0.0005,0.002))),
                                                                        (log(1-exp(theta.10[mm,m+1,j]+theta.11[mm,m+1,j])) - 
                                                                        log(exp(theta.10[mm,m+1,j])+exp(theta.11[mm,m+1,j]) + 2*exp(theta.10[mm,m+1,j]+theta.11[mm,m+1,j])))))
        ...
        for(ii in 1:J){
          if((number[ii]==0)==FALSE){
            for(iii in 1:number[ii]){
              ...
              theta.10[mm,m+1,iii+n]=theta.10[mm,m+1,ii]
              theta.11[mm,m+1,iii+n]=theta.11[mm,m+1,ii]
              theta.12[mm,m+1,iii+n]=theta.12[mm,m+1,ii]
            }}}
        }
  #Initial parameters for next iteration
  theta.10[mm+1,1,] = sapply(1:J,function(j) theta.10[mm+1,1,j] = theta.10[mm,M+1,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm > 50,0.001,0.005))))
  theta.11[mm+1,1,] = sapply(1:J,function(j) theta.11[mm+1,1,j] = min(theta.11[mm,M+1,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm > 50,0.0001,0.001))),-theta.10[mm,M+1,j]))
  theta.12[mm+1,1,] = sapply(1:J,function(j) theta.12[mm+1,1,j] = min(theta.12[mm,M+1,j] + rnorm(1,0,sd = sqrt(0.92^(((m-1)+(mm-1)*M)/(50*M)) * ifelse(mm > 50,0.0001,0.001))),
                                                                      (log(1-exp(theta.10[mm,M+1,j]+theta.11[mm,M+1,j])) - 
                                                                       log(exp(theta.10[mm,M+1,j])+exp(theta.11[mm,M+1,j]) + 2*exp(theta.10[mm,M+1,j]+theta.11[mm,M+1,j])))))
}

par(mfrow=c(1,3))
plot(rowMeans(logit(theta.10[,1,])),ylim=c(min(logit(theta.10)),0.2),type='l',col=1,main = 'Initial proportion of Susceptible',xlab = 'Iteraion times',ylab = 'I(S)')
lines(sapply(1:MM,function(mm) median(logit(theta.10[mm,1,]),0.1)),col=2,lty=1)
lines(sapply(1:MM,function(mm) quantile(logit(theta.10[mm,1,]),0.1)),col=3,lty=2)
lines(sapply(1:MM,function(mm) quantile(logit(theta.10[mm,1,]),0.9)),col=3,lty=2)
...
plot(rowMeans(logit(theta.11[,1,])),ylim=c(min(logit(theta.11)),0.0015),type='l',col=1,main = 'Initial proportion of Exposed',xlab = 'Iteraion times',ylab = 'I(E)')
lines(sapply(1:MM,function(mm) median(logit(theta.11[mm,1,]),0.1)),col=2,lty=1)
lines(sapply(1:MM,function(mm) quantile(logit(theta.11[mm,1,]),0.1)),col=3,lty=2)
lines(sapply(1:MM,function(mm) quantile(logit(theta.11[mm,1,]),0.9)),col=3,lty=2)
lines(sapply(1:MM,function(mm) 0.00027),col=4,lty=2)
legend("topright", bty ="n",legend=c("Est. I(S):mean","Est. median",'Est. 10%,90% quantile',"True Value"),col=c(1,2,3,4), lty=c(1,1,2,2), cex=0.7)

plot(rowMeans(logit(theta.12[,1,])),ylim=c(min(logit(theta.12)),0.0015),type='l',col=1,main = 'Initial proportion of Infectious',xlab = 'Iteraion times',ylab = 'I(I)')
lines(sapply(1:MM,function(mm) median(logit(theta.12[mm,1,]),0.1)),col=2,lty=1)
lines(sapply(1:MM,function(mm) quantile(logit(theta.12[mm,1,]),0.1)),col=3,lty=2)
lines(sapply(1:MM,function(mm) quantile(logit(theta.12[mm,1,]),0.9)),col=3,lty=2)
