...
N = sapply(1:53,function(m) sapply(1:J,function(j) sum(X[m,2:(SSS+1),j,14]))) 
plot(rowMeans(N),ylim=c(0,7000),type = 'l',main = 'London',xlab = 'Times(week)',ylab='Weekly Recovered')
lines(sapply(1:M, function(m) median(N[m,])),col=2,lty=1)
lines(sapply(1:M, function(m) quantile(N[m,],0.1)),col=3,lty=2)
lines(sapply(1:M, function(m) quantile(N[m,],0.9)),col=3,lty=2)
